...
# 多光谱（蓝光）数据集路径
MSBlue_train_path = '/public/home/hpc2420083500103/datasets/MSPalm/Blue/'
MSBlue_test_path = '/public/home/hpc2420083500103/datasets/MSPalm/Blue/'


# 处理多光谱（蓝光）掌纹数据集
class GetDataTextForMSBlue:
    """处理多光谱（蓝光）掌纹数据集::
        INPUTS:
            train_path：训练数据集路径
            test_path：测试数据集路径
    ...
    def __init__(self, train_path, test_path, root_path):
        super(GetDataTextForMSBlue, self).__init__()
        self.train_path = train_path    # 训练数据集路径
        self.test_path = test_path      # 测试数据集路径

        self.root_path = root_path    # 根路径

        self.save_train_path = self.root_path + 'train_MSBlue.txt'   # 训练文件路径
        self.save_test_path = self.root_path + 'test_MSBlue.txt'    # 测试文件路径
        self.save_verify_path = self.root_path + 'verify_MSBlue.txt'  # 验证文件路径
    ...
    # 得到数据集文本
    def get_data_text_for_MSBlue(self):
...
if __name__ == '__main__':
    ...
    # 多光谱（绿光）掌纹数据集处理
    (GetDataTextForMSBlue(train_path=MSBlue_train_path, test_path=MSBlue_test_path, root_path=root)
     .get_data_text_for_MSBlue())
