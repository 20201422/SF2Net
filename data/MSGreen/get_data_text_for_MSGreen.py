...
# 多光谱（绿光）数据集路径
MSGreen_train_path = ('/public/home/hpc2420083500103/datasets/MSPalm/Green/')
MSGreen_test_path = ('/public/home/hpc2420083500103/datasets/MSPalm/Green/')


# 处理多光谱（绿光）掌纹数据集
class GetDataTextForMSGreen:
    """处理多光谱（绿光）掌纹数据集::
        INPUTS:
            train_path：训练数据集路径
            test_path：测试数据集路径
    ...
    def __init__(self, train_path, test_path, root_path):
        super(GetDataTextForMSGreen, self).__init__()
        self.train_path = train_path    # 训练数据集路径
        self.test_path = test_path      # 测试数据集路径

        self.root_path = root_path    # 根路径

        self.save_train_path = self.root_path + 'train_MSGreen.txt'   # 训练文件路径
        self.save_test_path = self.root_path + 'test_MSGreen.txt'    # 测试文件路径
        self.save_verify_path = self.root_path + 'verify_MSGreen.txt'  # 验证文件路径
    ...
    # 得到数据集文本
    def get_data_text_for_MSGreen(self):
...
if __name__ == '__main__':
    ...
    # 多光谱（绿光）掌纹数据集处理
    (GetDataTextForMSGreen(train_path=MSGreen_train_path, test_path=MSGreen_test_path, root_path=root)
     .get_data_text_for_MSGreen())
