...
# 多光谱（近红外光）数据集路径
MSNIR_train_path = '/public/home/hpc2420083500103/datasets/MSPalm/NIR/'
MSNIR_test_path = '/public/home/hpc2420083500103/datasets/MSPalm/NIR/'


# 处理多光谱（近红外光）掌纹数据集
class GetDataTextForMSNIR:
    """处理多光谱（近红外光）掌纹数据集::
        INPUTS:
            train_path：训练数据集路径
            test_path：测试数据集路径
    ...
    def __init__(self, train_path, test_path, root_path):
        super(GetDataTextForMSNIR, self).__init__()
        self.train_path = train_path    # 训练数据集路径
        self.test_path = test_path      # 测试数据集路径

        self.root_path = root_path    # 根路径

        self.save_train_path = self.root_path + 'train_MSNIR.txt'   # 训练文件路径
        self.save_test_path = self.root_path + 'test_MSNIR.txt'    # 测试文件路径
        self.save_verify_path = self.root_path + 'verify_MSNIR.txt'  # 验证文件路径
    ...
    # 得到数据集文本
    def get_data_text_for_MSNIR(self):
...
if __name__ == '__main__':
    ...
    # 多光谱（近红外光）掌纹数据集处理
    (GetDataTextForMSNIR(train_path=MSNIR_train_path, test_path=MSNIR_test_path, root_path=root)
     .get_data_text_for_MSNIR())
