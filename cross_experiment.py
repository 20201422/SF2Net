...
if __name__ == "__main__":
    ...
    ##Store Path
    parser.add_argument("--des_path", type=str, default='./results/cross_experiment/Tongji/checkpoint/')
    parser.add_argument("--path_rst", type=str, default='./results/cross_experiment/Tongji/rst_test/')
    ...
    source_model = "./results/" + source_id + "/checkpoint/net_params_best.pth"  # 源模型路径
    ...
    print('------Init Model------')
    best_net = SF2Net(label_num=label_num, vit_floor_num=3)
    best_net = type(best_net)(label_num, 3)  # 创建一个新的模型实例
    best_net.load_state_dict(torch.load(source_model))  # 加载最好的模型状态字典
    best_net.cuda()
